...
async def async_mock_service(
    hass: HomeAssistant, domain: str, service: str, return_value: Any = None
) -> list[dict[str, Any]]:
    """Mock a service and return a list to track calls."""
    calls = []

    async def mock_service_handler(call: Any) -> Any:
        calls.append(
            {
                "domain": call.domain,
...
def mock_restore_cache(hass: HomeAssistant, states: list[dict[str, Any]]) -> None:
    ...
    hass.data[RESTORE_DATA] = {"restore_state_data": data}


async def async_setup_component(
    hass: HomeAssistant, domain: str, config: dict[str, Any] | None = None
) -> bool:
...
def assert_setup_component(count: int, domain: str) -> Any:
    ...
    async def mock_setup(hass: HomeAssistant, config_input: dict[str, Any]) -> bool:
        """Mock setup function."""
