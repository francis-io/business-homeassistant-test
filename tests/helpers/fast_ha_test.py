...
class FastHATest:
    ...
    async def start(self) -> HomeAssistant:
        ...
        await asyncio.gather(
            ...
            async_setup_component(self.hass, 'light', {
                'light': {
                    'platform': 'demo'
                }
            }),
            async_setup_component(self.hass, 'binary_sensor', {
                'binary_sensor': {
                    'platform': 'demo'
                }
            }),
            async_setup_component(self.hass, 'climate', {
                'climate': {
                    'platform': 'demo'  
                }
            }),
            async_setup_component(self.hass, 'person', {}),
            async_setup_component(self.hass, 'zone', {
                'zone': {
                    'name': 'Home',
                    'latitude': 40.7128,
                    'longitude': -74.0060,
            ...
            async_setup_component(self.hass, 'notify', {
                'notify': [{
                    'platform': 'demo',
                    'name': 'test_notifier'
                }]
    ...
    async def add_automation(self, automation_config: Dict[str, Any]):
        ...
        await async_setup_component(self.hass, 'automation', {
            'automation': self._automations
        })
        await self.hass.async_block_till_done()
    ...
    async def advance_time(self, seconds: int = 0, minutes: int = 0):
        """Advance time for testing time-based automations."""
        # Fire time changed event
        next_time = dt_util.utcnow() + datetime.timedelta(seconds=seconds, minutes=minutes)
    ...
    def get_service_calls(self, domain: str, service: str) -> List[Dict]:
