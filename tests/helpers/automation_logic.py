...
def should_trigger_time_pattern(
    current_minute: int,
    pattern_minutes: int
) -> bool:
    ...
    if person_state == "home" and previous_state != "home":
        ...
        if time(17, 0) <= current_time <= time(22, 0):
            actions["lights"].extend([
                {"entity": "light.entrance", "action": "turn_on", "brightness": 80},
                {"entity": "light.hallway", "action": "turn_on", "brightness": 60}
            ])
        ...
        if people_home_count == 1:
            actions["lights"].append({
                "entity": "light.living_room",
                "action": "turn_on"
            })
            actions["security"].append({
...
def calculate_home_occupancy(
    ...
    people_home = [
        person for person, state in people_states.items()
        if state == "home"
    ]
...
def get_presence_based_climate_mode(
    people_home: int,
    current_hour: int
) -> Optional[str]:
