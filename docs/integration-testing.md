...
1. **Include ID field** - Required for proper automation identification
1. **Focus on behavior** - Test service calls and state changes
1. **Execute directly** - Don't expect automation entities
1. **Mock services** - Register handlers to track calls
...
1. **Minimize setup** - Share fixtures when possible
1. **Batch similar tests** - Group by automation type
1. **Skip when appropriate** - Use pytest marks for conditional tests
